...
# model = load_model('../_data/diamond-neural-network.h5')
# pca = joblib.load('../_data/pca.joblib')
# scaler = joblib.load('../_data/scaler-missing.joblib')
model = load_model('./Chapter9/Model/diamond-neural-network.h5')
pca = joblib.load('./Chapter9/Model/pca.joblib')
scaler = joblib.load('./Chapter9/Model/scaler-missing.joblib')
# We have to do this due to some Keras' issue
model._make_predict_function()

# Build the objects that will receive the user's inputs
# Six inputs for the numerical values
## Div for carat
...
# three inputs for the categorical values
## Div for cut
...
# Group nine inputs into three groups
...
# Function to take the values from the user and produce the price prediction
...
# Build the decorator (callback) that will update the output using the nine inputs
